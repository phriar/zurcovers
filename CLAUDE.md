...
**Missing-rarities / "Complete Set" detection** (on the Long Box, per collection) compares which rarity tiers a wallet owns for a collection against which tiers Magic Eden currently has *any* active listing for (via the Worker's `/v2/collections/{symbol}/rarity-summary`, see Architecture below). This replaced an earlier "Complete the Run" feature that tried to guess missing *issues* from gaps between the lowest/highest owned issue number — that broke badly on anthology/reprint series (e.g. "Showcase (1956-1978)"), where a wallet might own a couple of scattered issues decades apart, reporting hundreds of nonexistent "missing" books. Rarity tiers are a small fixed set per collection, so this can't blow up the same way. Checked both lazily (when a collection's panel is opened) and eagerly (folded into the same per-collection loading loop that resolves floor prices, so a "Complete Set" ribbon can show on the shelf card itself without opening every collection) — see `ensureRarityFloors`/the eager loop in `MyComics.html`.
...
- `GET /v2/collections/{symbol}/rarity-summary` — a KV-cached (`ZURCOVERS_CACHE`, `RARITY_SUMMARY_CACHE_TTL_SECONDS` = 30 min) wrapper around that same collection's `/listings` data, same response shape. Backs MyComics.html's per-collection "missing rarities" / "complete set" check. Deliberately KV rather than the 60s Cache-API edge cache above: "which rarity tiers currently have any active listing" isn't wallet-specific, so it's worth caching much longer and sharing globally across every visitor checking that collection — the site gets faster at this over time as more collections get warmed into cache, which the per-datacenter Cache API can't do.
